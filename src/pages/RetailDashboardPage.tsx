    ...
    <div className="space-y-6 pb-12 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-6 flex-1 flex flex-col justify-center text-center relative overflow-hidden">
             <div className="absolute top-0 inset-x-0 h-1.5 bg-emerald-500"></div>
             <Battery className="w-12 h-12 text-emerald-500 mx-auto mb-3" />
             <h2 className="text-3xl font-bold text-slate-900 dark:text-white">88%</h2>
             <p className="text-sm font-semibold text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-wider">System Health</p>
             
             <div className="mt-6 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl">
               <p className="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider mb-1">Estimated Lifespan</p>
               <p className="text-xl font-bold text-slate-900 dark:text-white">6.5 Years</p>
             </div>
          </div>
        </div>
      ...
      <div className="bg-gradient-to-r from-amber-50 to-white dark:from-amber-900/20 dark:to-slate-800 rounded-2xl border border-amber-200 dark:border-amber-700/50 shadow-sm p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-start gap-4">
          <div className="p-3 bg-amber-100 text-amber-600 dark:bg-amber-900/50 dark:text-amber-400 rounded-xl shrink-0">
            <AlertTriangle className="w-8 h-8" />
          </div>
          <div>
            <h2 className="text-lg font-bold text-amber-900 dark:text-amber-400">Minor Voltage Fluctuation Detected</h2>
            <p className="text-amber-800 dark:text-amber-300/80 text-sm mt-1 max-w-2xl">
              Replacing the $45 capacitor now will extend the life of your pump by 3 years. Prevent a $2,000 emergency replacement.
            </p>
          </div>
        </div>
        
        <Link to="/marketplace" className="shrink-0 bg-amber-500 hover:bg-amber-600 text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 transition-colors shadow-sm">
